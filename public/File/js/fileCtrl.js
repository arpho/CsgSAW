...
angular.module('csgSAW.controllers').controller('FilesController', ['$scope', 'UserService', '$mdMedia', '$mdDialog',
    ...
    function ($scope, $rootScope, $mdDialog, Messages, FileService, Schools, User) {
        ...
        var self = this,
            data = User.generateDataPayload(), // preparo la richiesta al server per la lista delle scuole
        ...
        $scope.submit = function (registrazione) {
            var path = FileService.buildRelativePath(registrazione),
                nomeFile = buildName(registrazione)
            var data = {
                relativePath: path,
                nomeFile: nomeFile,
                estensione: registrazione.estensione
            }
